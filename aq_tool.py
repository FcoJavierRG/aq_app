...
# ================================================================
# 5. Visualization (Route Drawing)
# ================================================================
def draw_routes_on_image(img: np.ndarray, G: nx.Graph, routes: List[List[int]], color=(0, 0, 255)) -> np.ndarray:
    """Draw all route paths over the original image."""
    img_draw = img.copy()
    for route in routes:
        for i in range(len(route) - 1):
            ...
            if data and "path" in data:
                for (y, x) in data["path"]:
                    cv2.circle(img_draw, (int(x), int(y)), 1, color, -1)
            else:
                y1, x1 = int(G.nodes[u]["y"]), int(G.nodes[u]["x"])
                y2, x2 = int(G.nodes[v]["y"]), int(G.nodes[v]["x"])
                cv2.line(img_draw, (x1, y1), (x2, y2), color, 1)
    return img_draw
...
# ================================================================
# 8. Geometry-based AccessQuotient + Routes
# ================================================================
